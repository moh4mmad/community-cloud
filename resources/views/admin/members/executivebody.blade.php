@extends('admin.app')
@section('title') Executive Body @endsection
...
<div class="modal fade" id="edit{{$member->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" style="" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form enctype="multipart/form-data" action="{{ route('admin.executivebody.update') }}" method="post">
			{{ csrf_field() }}
		 <input type="hidden" name="id" value="{{$member->id}}">
          ...
          <div class="form-group">
		   ...
		   <div class="form-group">
		   <label>Image:</label>
		   <input type="file" name="image" id="input-file-now" data-height="150" class="dropify" data-default-file="@if($member->image == null){{asset('assets/img/no-image.png')}}@else{{asset('assets/img/executivebody/')}}/{{$member->image}}@endif">
           </div>
		  <div class="form-group">
            <label for="title-name" class="col-form-label">Content:</label>
		  ...
		  <div class="form-group">
            <label for="title-name" class="col-form-label">Linkedin url:</label>
            <input type="text" class="form-control" value="{{$member->linkedin_url}}" name="linkedin_url">
          </div>
...
$("a#remove").click(function(){
    ...
    var memberid = $(this).attr("memberid");
 	$.msgbox("Are you sure that you want to permanently delete this member?", {
	   ...
	   $.post("{{ route('admin.executivebody.remove') }}",
       {
         id: memberid,
         _token: '{{ csrf_token() }}',
       },
...
$('#addmember').on('submit',function(event){
       ...
       $.ajax({
        url: "{{ route('admin.executivebody.add') }}",
        type: "post",
